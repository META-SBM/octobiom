...
features <- c('bmi','bmi_group','age_at_baseline','age_group',
              'sex',
              "kidney.disease","chronic_bronchitis","smoking_status","YEAR",
              "diabetes" ,"tuberculosis" )

#You can change number of rows and cols in  combined plot
meta_fig_1 <- octobiom::plot_metadata_distributions(ps,features = features,color_palette = color_palette,nrow =3,ncol = 4)
meta_fig_1
```
#Prepare phyloseq
...
plot(permanova_results[[2]])
```
#Alpha
There are many views in ecology and microbiology about how best to characterize biological diversity. One of the most common approaches is to estimate α-diversity, which reflects the species richness and evenness of distribution of species within a community. We provide the ability to calculate the Gini-Simpson, Shannon, chao1 index. In the first two cases, composite data is required. Therefore, the indices are calculated separately and then combined. You can pass either one variable for study or several
...
plot_alpha_list <- purrr::map(features, ~ {
  tryCatch({
    octobiom::create_alpha_plots(ps_obj, col= .x, colors = color_palette,measure = c("Shannon",'Simpson'),bracket_nudge_y = 0.1)
  }, error = function(e) {
...
plot_alpha_list_chao <- purrr::map(features, ~ {
  tryCatch({
    octobiom::create_alpha_plots(ps_obj, col= .x, colors = color_palette,measure = c("Chao1"),bracket_nudge_y = 1)
  }, error = function(e) {
    message(paste("Error in method:", .x))
    message(e$message)  # Print the error message
    return(NULL)  # Return NULL for this iteration
...
It is important to assess the distribution of bacteria across samples to evaluate potential batch effects and examine sample clustering by groups. For this purpose, we propose using a hierarchical clustering function.
...
colors_annotation <- list('YEAR' = YEAR_colors,'bmi_group' = bmi_colors,
                                                                     'age_group' = age_colors,'sex' = sex_colors)
hier_fig <- octobiom::barplot_hierarchical(ps_obj,taxrank="Species", top = 20, feature = features,
                                      dist = 'bray',colors_barplot = color_palette,
                                   colors_annotation = colors_annotation,decreasing = T,
                                   size = c(0.5,0.2,0.2,0.2,0.2,5))
hier_fig$combined_plot
```
#correlation between features and taxa
```{r,fig.height=4,fig.width=6}
?octobiom::prepare_phyloseq

ps_obj <- octobiom::prepare_phyloseq(ps,transformation = 'clr',detection = 10,prev = 0.20,taxonomic_level = 'Species',
                                     first = 'filter')
...
network_correlation_results <- octobiom::build_multi_correlation_network(
  ...
  tax_rank = "Species",
  cor_method = 'pearson',
  feature_colors = feature_colors,min_corr_strenght = 0.2
  
)
network_correlation_results[[2]]
```
