...
ps_obj <- octobiom::prepare_phyloseq(ps_Hadza, transformation = 'compositional',detection = 0, prevalence= 0,
...
unique(ps_obj@sam_data$age_group)
ps_obj@sam_data$bmi_group

sample_data(ps_obj) <- as(sample_data(ps_obj),'data.frame') %>%
  #as.data.frame() %>%
...
plot_list <- octobiom::compare_prevalence_across_group(ps_obj,column='age_group',zoom_window = zoom_list )
plot_list
